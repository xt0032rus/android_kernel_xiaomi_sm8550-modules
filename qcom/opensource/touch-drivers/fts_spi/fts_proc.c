				     ...
				     size_t count, loff_t *pos)
{
	...
	funcToTest =
		(u32 *)kzalloc(sizeof(u32) * ((count + 1) / 3), GFP_KERNEL);
	if (!funcToTest) {
		res = -ENOMEM;
		goto END;
	}
	...
	if (numberParam >= 1) {
		...
		case CMD_FLASHERASEPAGE:
			if (numberParam == 2) { /* need to pass: keep_cx */
				...
				res = readFwFile(info->board->default_fw_name,
						 &fw, funcToTest[1]);
				if (res < OK)
					logError(
						1,
		...
		case CMD_MSRAWTEST:
			if (numberParam == 2)
				res = production_test_ms_raw(limit_file_name,
							     cmd[1], &tests);
			else {
				logError(1, "%s Wrong number of parameters! \n",
					 tag);
		...
		case CMD_MSINITDATATEST:
			if (numberParam == 2)
				res = production_test_ms_cx(limit_file_name,
							    cmd[1], &tests);
			else {
				logError(1, "%s Wrong number of parameters! \n",
					 tag);
		...
		case CMD_SSRAWTEST:
			if (numberParam == 2)
				res = production_test_ss_raw(limit_file_name,
							     cmd[1], &tests);
			else {
				logError(1, "%s Wrong number of parameters! \n",
					 tag);
		...
		case CMD_MAINTEST:
			if (numberParam == 3)
				res = production_test_main(limit_file_name,
							   cmd[1], cmd[2],
							   &tests);
			else {
				logError(1, "%s Wrong number of parameters! \n",
		...
		case CMD_GETLIMITSFILE:
			if (numberParam >= 1) {
				...
				lim.size = 0;
				if (numberParam == 1)
					res = getLimitsFile(limit_file_name,
							    &lim);
				else
